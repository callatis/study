...
public class ErrorRateMonitorAR implements ErrorRateMonitor {

    public static final int NUM_EXTRA_BUCKETS = 2; // current
    
    private final int numBuckets;
    private final AtomicReference<Snapshot>[] errors;
    private final AtomicReference<Snapshot>[] successes;
    ...
    private static class Snapshot {
        ...
        public Snapshot(long bucket, long counter) {
            this.bucket = bucket;
    ...
    public ErrorRateMonitorAR(int numBuckets) {
        this.numBuckets = numBuckets + NUM_EXTRA_BUCKETS;
        this.errors = new AtomicReference[this.numBuckets];
        this.successes = new AtomicReference[this.numBuckets];
        ...
        for (int i = 0; i < this.numBuckets; i++) {
            ...
            this.errors[inx] = new AtomicReference<>(new Snapshot(bucketTimeSec, 0)); // right 32 bits, representing # errors, are 0
            // 0 = 20:36:25, 1 = 20:36:26, 2 = 20:36:22, 3 = 20:36:23, 4 = 20:36:24
            this.successes[inx] = new AtomicReference<>(new Snapshot(bucketTimeSec, 0));
        }
    }
    ...
    private void recordEvent(AtomicReference<Snapshot>[] events, long currTimeSec) {
        int inx = (int) currTimeSec % this.numBuckets;
        while (true) {
            Snapshot currSnapshot = events[inx].get();
            long bucketTimeSec = currSnapshot.bucket;
            Snapshot newSnapshot;
            if (bucketTimeSec < currTimeSec) { // we're in an old bucket - attempt to set a new bucket with a counter = 1
                newSnapshot = new Snapshot(currTimeSec, 1);
            } else { // we're in the right bucket - just increment the counter, which is the same as incrementing the snapshot
                newSnapshot = new Snapshot(bucketTimeSec, currSnapshot.counter + 1);
            }
    ...
    private long addNumEvents(AtomicReference<Snapshot>[] events, int bucket, final long currTimeSec) {
        Snapshot snapshot = events[bucket].get();
        if (snapshot.bucket >= currTimeSec - this.numBuckets) { // active bucket
            return snapshot.counter; // extract the counter as the right int
        }
